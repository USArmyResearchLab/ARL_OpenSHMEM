...
define(decl_shmem_x_atomic_op,
`void shmem_$1_atomic_$3( $2* ptr, $2 val, int pe);'
)

define(func_shmem_x_atomic_op,
`void shmem_$1_atomic_$3( $2* ptr, $2 val, int pe)
{
	$2* remote_ptr = ($2*)shmem_ptr((void*)ptr, pe );
	long* x = (long*)shmem_ptr((void*)&__shmem.lock_atomic_$1, pe);
	__set_spin_lock(x);
	*remote_ptr = $4(*remote_ptr,val);
	__clear_spin_lock(x);
}'
)
